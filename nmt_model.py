...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        #TODO not sure if embed_size + hidden_size is correct
        self.encoder = nn.LSTM(embed_size, hidden_size, bidirectional=True)
        self.decoder = nn.LSTMCell(embed_size + hidden_size, hidden_size)
        #TODO not sure if embed_size + hidden_size is correct or just embed_size sel
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dhu = torch.unsqueeze(dec_hidden, dim=2)
        #print(dhu.shape)
        #print('enc_hiddens_proj.shape', enc_hiddens_proj.shape)
        e_t = torch.bmm(enc_hiddens_proj, dhu)
        #print(e_t.shape)
